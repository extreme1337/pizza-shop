...
class FrontendController extends Controller
{
    ...
    public function store(Request $request){
        if($request->small_pizza===0 && $request->medium_pizza===0 && $reqeust->large_pizza===0){
            return back()->with('message', 'Please order atleast one pizza');
        }
        if($request->small_pizza<0 || $request->medium_pizza<0 || $reqeust->large_pizza<0){
            return back()->with('message', 'Order should not have negative number');
        }
        ...
        Order::create([
            ...
            'large_piza'=>$request->large_pizza,
            'body'=>$request->body,
            'phone'=>$request->phone,
        ]);
